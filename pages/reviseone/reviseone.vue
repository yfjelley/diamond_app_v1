<template>
	<view>
		...
		<view class="fixed-btn">
			<view class="btn" @click="back" style="background-color: #ccc;">
				取消
			</view>
			<view class="btn" @click="con">
				确定
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			clear() {},
			async reviseInfo(){
				let res=await reviseGrid(this.detail.id,this.form)
				if(res.code==200){
					uni.showToast({
						title:'修改参数成功',
						icon:'none'
